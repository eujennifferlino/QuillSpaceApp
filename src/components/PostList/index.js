...
function PostList({ data,userId }){
  ...
  async function handleLikePost(id, likes){
    ...
    //Conferir se o post já tem like do usuário logado
    const doc = await firestore().collection('likes')
    .doc(docId).get();

    if(doc.exists){
      //O post já tem like e desse modo terá que remover
    ...
    //O post ainda não tem like e desse modo terá que acrescentar
  ...
  return(
    <Container>
      <Header onPress={ () => navigation.navigate("UserPosts", { title: data.autor, userId: data.userId }) }>
        {data.avatarUrl ? (
          <Avatar
